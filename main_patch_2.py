...
platform1 = cinetic_entity(25, 200, 25, 100, 4, image='platform_2.png')
platform1.initialise()
platform1.show()
platform2 = cinetic_entity(650, 200, 25, 100, 4, image='platform_2.png')
platform2.initialise()
platform2.show()
ball1 = ball(300, 225, 50, 50, 4, image='ball.png')
ball1.initialise()
ball1.show()
fuente = font.SysFont('Arial', 25)
fuente_small = font.SysFont('Arial', 15)

#Variables que condicional el programa y serán usadas posteriormente.
end_condition = False
timer = time.Clock()
is_point = False
last_colision_time = time_screenshot()
player1_points, player2_points = 0, 0

def ball_collide(object, time=last_colision_time):
    global last_colision_time
    act_time = time_screenshot()
    elapsed_time = act_time - last_colision_time
    if ball1.collide_area.colliderect(object.collide_area) and elapsed_time > 0.2:
        last_colision_time = act_time
        if object == platform1:
            if ball1.xpos >= object.xpos + 19:
                ball1.xspeed *= -1
            if (ball1.ypos + 25) >= (object.ypos + 50):
                ball1.yspeed = 4
            else: 
                ball1.yspeed = -4
        if object == platform2:
            if ball1.xpos + 40 < object.xpos:
                ball1.xspeed *= -1
            if (ball1.ypos + 25) >= (object.ypos + 50):
                ball1.yspeed = 4
            else: 
...
def update_screen():
    ...
    platform2.show()
    ball1.show()
    p1_points = fuente_small.render(f'Puntos del jugador 1: {player1_points}', True, (255,255,255))
    p2_points = fuente_small.render(f'Puntos del jugador 2: {player2_points}', True, (255,255,255))
    mwin.blit(p1_points, (275, 0))
    mwin.blit(p2_points, (275, 25))
    display.update()
...
while not end_condition:
    ...
    if ball1.xpos > -4 and ball1.xpos < 654:
        ...
        ball1.xspeed *= -1
        ball1.horizontal_step()
    if ball1.ypos > -4 and ball1.ypos < 454:
        ball1.vertical_step()
    else:
        ball1.yspeed *= -1
        ball1.vertical_step()
    
    #Colisión de la bola con la plataforma.
    ball_collide(platform1)
    ball_collide(platform2)

    #Manejo de puntos.
    if is_point:
        if ball1.xpos < 100:
            text = 'Punto del jugador 2'
            player2_points += 1
        else: 
            text = 'Punto del jugador 1'
            player1_points += 1
        if player1_points == 3 or player2_points == 3:
            ...
            update_screen()
            msg = fuente.render(text, True, (0, 0, 255))
            mwin.blit(msg, (260, 205))
            display.update()
            end_condition = True
            sleep(3)
        else:
            ...
            msg = fuente.render(text, True, (0, 0, 255))
            mwin.blit(msg, (240, 205))
            display.update()
            var = 1
            for i in range(5):
                sleep(0.2)
            # Decido la direción de la pelota.
            if randint(0,1) == 1:
                ball1.xspeed = -4
